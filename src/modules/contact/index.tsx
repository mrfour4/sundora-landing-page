    ...
    >
      <div className="space-y-2 lg:space-y-6">
        <div className="flex gap-x-3">
          <MapPin className="mt-1 size-4 fill-sundora-secondary" />
          <ul className="space-y-0 lg:space-y-3">
            {CONTACT_LOCATIONS.map((c, index) => (
              <li key={index}>
                <ContactItem href="#!" label={c.office} content={c.address} />
              </li>
            ))}
          </ul>
        </div>
        <ul className="lg:space-y-3">
          {CONTACTS.map((c, index) => (
            <li key={index} className="flex items-start gap-x-3">
              <c.icon className="mt-1 size-4 fill-sundora-secondary" />
              <ContactItem {...c} />
            </li>
          ))}
        </ul>
