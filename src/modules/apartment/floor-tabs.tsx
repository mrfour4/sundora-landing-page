...
export const FloorTabs = ({ count, active, onChange, className }: Props) => {
  ...
  return (
    <div className={cn('relative w-fit pb-2', className)}>
        ...
        {Array.from({ length: count }).map((_, i) => (
          <div
            ...
            className={cn(
              ...
              i === active &&
                'text-sundora-sundora-secondary-foreground font-bold'
            )}
          >
            {`TẦNG ${i + 1}`}
          </div>
      ...
      <div className="bg-muted-secondary pointer-events-none absolute right-0 bottom-0 left-0 h-px -translate-y-1.5" />
      <motion.div
        className="absolute h-1 bg-sundora-primary"
        layout
