...
export const VRButton = ({ className, showDetail = false }: Props) => {
  return (
    <motion.div
      ...
      className={cn('inline-flex rounded-full border-1 text-white', className)}
    >
      <Button
        variant="ghost"
        className={cn(
          'cursor-pointer rounded-full border-1 border-sundora-secondary text-xs font-medium text-inherit hover:bg-transparent! hover:text-sundora-secondary lg:text-sm',
